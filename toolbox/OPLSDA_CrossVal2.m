function [error_fraction]=OPLSDA_CrossVal2(X,y,nlv,nvals,preprocessing,output);
%   in/out:
%   [error_fraction]=OPLSDA_CrossVal2(X,y,nlv,nvals,preprocessing,output);
%
%   Cross validation to determine rank of OPLS-DA models.
%   Cross validation method is 'leave one randomly selected sample of each 
%   class out'. These 2 samples are used as test set. This procedure is 
%   repeated 'nvals' times. The 'error_fraction' is based on the
%   classification outcome of these 'nvals' test sets.
%   If output=1 a plot is produced of the error_fraction as function of the
%   number of latent variables up to nlv+1.
%
% output:
%   error_fraction      Fraction of erroneously classified samples per # of LVs
% input:
%   X                   data (no preprocessing)
%   y                   class (no preprocessing)
%   nlv                 maximum # of Y-ORTHOGONAL LVs for analysis
%   nvals               # of validations
%   preprocessing       Which preprocessing? : 0=nothing, 1=mean centering, 2= autoscaling
%   output              0 if no plot, default is 1
%
% GJP: modified: autosc ipv nanauto
...
errorA=zeros(nlv+1,1);
errorB=zeros(nlv+1,1);
...
for ival=1:nvals
   ...
   %% generate training and test set
    ioutA=randperm(length(iA));
    ioutB=randperm(length(iB));
    ioutA=ioutA(1);
    ioutB=ioutB(1);
    ...
    %% GJP 2013: also 0 LVs: only Y-related part
    for ilv=0:nlv;
        ...
        if(distclass1(2)<=distclass1(1))
            % GJP errorA(ilv)=errorA(ilv)+1;
            errorA(ilv+1)=errorA(ilv+1)+1;
            CV_Error_Sample(outA)=CV_Error_Sample(outA)+1;
        end
        if(distclass2(1)<=distclass2(2))
            errorB(ilv+1)=errorB(ilv+1)+1;
...
if output~=0
    ...
    title('OPLS LV optimization through Cross Validation')
end
